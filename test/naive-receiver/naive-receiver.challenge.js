...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Execution', async function () {
      ...
      [deployer, user, player] = await ethers.getSigners();
      const AttackerFactory = await ethers.getContractFactory('NaiveFlashLoanReceiverAttacker', player);
      const attacker = await AttackerFactory.deploy();
      expect(await attacker.connect(player).attack(pool.address, receiver.address, ETH));
    });
